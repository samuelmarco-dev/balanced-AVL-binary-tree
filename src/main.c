...
node *insert(node *tree, int value) {
    ...
    int balance = setBalance(tree);

    if(balance > 1 && value < info(left(tree))) return rotateRight(tree);
    if(balance < -1 && value > info(right(tree))) return rotateLeft(tree);
    if(balance > 1 && value > info(left(tree))) {
        setNodeLeft(tree, rotateLeft(left(tree)));
        return rotateRight(tree);
    }
    if(balance < -1 && value < info(right(tree))) {
        setNodeRight(tree, rotateRight(right(tree)));
...
int height(node *n) {
    if(n == NULL) return -1;
    if(left(n) == NULL && right(n) == NULL) return 0;

    return max(height(left(n)), height(right(n))) + 1;
}

int setBalance(node *n) {
    return n == NULL ? 0 : (height(left(n)) - height(right(n)));
}

node *rotateLeft(node *n) {
    node *newRoot = right(n);
    n->right = left(newRoot);
    newRoot->left = n;
